...
// All dates are local-time and DST-safe: constructed from components.
...
test('grid: 30-minute steps inside 09:00-18:00', () => {
  ...
  assert.deepEqual(
    slots.map((s) => [s.start.getHours(), s.start.getMinutes()]),
...
test('a slot is offered only if it fits into the working window', () => {
  ...
  assert.equal(last.start.getHours(), 17);
  assert.equal(last.start.getMinutes(), 0); // 17:00 + 60min = 18:00, fits
});
...
test('busy intervals are excluded', () => {
  ...
  assert.equal(
    slots.some((s) => s.start.getHours() === 9 && s.start.getMinutes() === 30),
    false,
  );
